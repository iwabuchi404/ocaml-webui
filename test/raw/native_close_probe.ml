...
let run () =
  ...
  require
    (Webui_raw.Window.set_html window
       "<!doctype html><title>WM_CLOSE probe</title><script>window.hold()</script>");
  let closer =
    Thread.create
      (fun () ->
        try
          ...
          require (Webui_raw.For_testing.Win32.post_wm_close window);
  ...
  | None -> failwith "WM_CLOSE Thread did not publish its outcome");
  if Webui_raw.Window.state window <> Stopped then
    failwith "WM_CLOSE did not stop the Window";
  (match Atomic.get captured_call with
  | Some call when Webui_raw.For_testing.call_state call = Cancelled -> ()
  | _ -> failwith "WM_CLOSE did not cancel the pending Call");
  ...
  then failwith "WM_CLOSE retained pending work";
  ...
  then failwith "WM_CLOSE retained roots after destroy";
  Printf.printf "native_close_probe: passed WM_CLOSE pending_cancel roots=0\n%!"
